...
export default function CrewContractsTab({ contact }) {
  const [contracts, setContracts] = useState([]);
  const [loading, setLoading]     = useState(true);
  const [viewing, setViewing]     = useState(null);

  useEffect(() => {
    base44.entities.Contract.filter({ contact_name: contact.name })
      .then(cs => setContracts(cs.filter(c => c.status !== 'draft')))
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: '#444' }}>
      <div style={{ width: 24, height: 24, border: '3px solid #333', borderTopColor: '#E81A1A', borderRadius: '50%', animation: 'spin 0.7s linear infinite', margin: '0 auto' }} />
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>My Contracts</div>
        <div style={{ fontSize: 13, color: '#555' }}>
          {contracts.length === 0 ? 'No contracts yet.' : `${contracts.length} contract${contracts.length !== 1 ? 's' : ''}${pending ? ` · ${pending} awaiting signature` : ''}`}
        </div>
      </div>
      ...
      {pending > 0 && (
        <div style={{ marginBottom: 20, padding: '14px 18px', background: 'rgba(74,158,255,0.07)', border: '1px solid rgba(74,158,255,0.25)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#666' }}>Tap to review and sign</div>
          </div>
        </div>
      )}
