...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#fff', fontFamily: 'Syne, sans-serif' }}>
      ...
      {/* Header */}
      <header style={{ borderBottom: '1px solid #1E1E1E', padding: '0 24px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(10,10,10,0.95)', backdropFilter: 'blur(8px)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 24, height: 56 }}>
          <div style={{ fontSize: 20, fontWeight: 800, flexShrink: 0 }}>Studio <span style={{ color: '#E81A1A' }}>65</span></div>
          <nav style={{ display: 'flex', gap: 2, flex: 1 }}>
            {TABS.map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                padding: '6px 14px', borderRadius: 6, fontSize: 13, fontWeight: 600,
                cursor: 'pointer', background: tab === t ? '#1E1E1E' : 'transparent',
                color: tab === t ? '#fff' : '#666', border: 'none', transition: 'all 0.15s',
              }}>{t}</button>
            ))}
          </nav>
          {tab === 'Projects' && (
            <button onClick={() => { setEditingProject(null); setProjectModalOpen(true); }} style={{
              padding: '7px 16px', background: '#E81A1A', border: 'none', borderRadius: 8,
